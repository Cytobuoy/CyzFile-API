...
''' </summary>
''' <remarks></remarks>
<TestClass()> Public Class TestImageProcessing


    Private ReadOnly imgProcSettings As CytoSense.Data.ParticleHandling.CytoImage.ImageProcessingSettings = New ParticleHandling.CytoImage.ImageProcessingSettings() With { 
        .MarginBase=25, .MarginFactor=0.1, .Threshold = 9, .ErosionDilation = 1, .ApplyBrightFieldCorrection = True, .ExtendObjectDetection = True }

    ''' <summary>
    ''' 
    ''' </summary>
    <DataTestMethod()>
    Public Sub TestCropping_profiles_LW_2017()
        Dim dfw = New DataFileWrapper("DataFiles/profiles_LW_2017_protocol_1 2017-04-20 12h00_New Set 2.cyz")

        Dim imgParticles = dfw.SplittedParticlesWithImages()
        Assert.AreEqual(11, imgParticles.Count)
        For imgIdx As Integer = 0 To imgParticles.Count - 1
            Assert.AreEqual(False, imgParticles(imgIdx).ImageHandling.IsCropped)
            ' Dim img = imgParticles(imgIdx).ImageHandling.GetCroppedImage(imgProcSettings)
            ' Console.WriteLine("ID={0}, WxH=({1} x {2})", imgParticles(imgIdx).ID, img.Cols, img.Rows)
        Next
    ...
    Private Sub AssertImgParticle(imgPart As ParticleHandling.ImagedParticle, pId As Integer, w As Integer, h As Integer)
        Dim img = imgPart.ImageHandling.GetCroppedImage(imgProcSettings)
        Assert.AreEqual(pId, imgPart.ID)
        Assert.AreEqual(w, img.Width)
        Assert.AreEqual(h, img.Height)
    End Sub

    Private Sub AssertImgParticleWithRect(imgPart As ParticleHandling.ImagedParticle, pId As Integer, w As Integer, h As Integer, x As Integer, y As Integer)
        Dim img_rect = imgPart.ImageHandling.GetCroppedImageWithRect(imgProcSettings)
        Dim img = img_rect.Item1
        Dim rect = img_rect.Item2
        ' Console.WriteLine("ID={0},  X={1}, Y={2}", imgPart.ID, rect.Left, rect.Top)
        Assert.AreEqual(pId, imgPart.ID)
        Assert.AreEqual(w, img.Width)
        Assert.AreEqual(h, img.Height)
        ...
        Assert.AreEqual(x, rect.Left)
        Assert.AreEqual(y, rect.Top)
    End Sub


    <DataTestMethod()>
    Public Sub TestCroppingWithRect_profiles_LW_2017()
        Dim dfw = New DataFileWrapper("DataFiles/profiles_LW_2017_protocol_1 2017-04-20 12h00_New Set 2.cyz")
    ...
    End Sub
        ...
        <DataTestMethod()>
    Public Sub TestCropping_LiveCropped_ZOO80()
        Dim dfw = New DataFileWrapper("DataFiles/ZOO80um_50t60m_diluted5x_8uls_SWST50L60_5min_IIFall_sh high_smrttr SWS SL300 SWS max1000 2025-12-18 13h02.cyz")

        Dim imgParticles = dfw.SplittedParticlesWithImages()
        Assert.AreEqual(10, imgParticles.Count)
        For imgIdx As Integer = 0 To imgParticles.Count - 1
            Assert.AreEqual(True, imgParticles(imgIdx).ImageHandling.IsCropped)
            ' Dim img = imgParticles(imgIdx).ImageHandling.GetCroppedImage(imgProcSettings)
            ' Console.WriteLine("ID={0}, WxH=({1} x {2})", imgParticles(imgIdx).ID, img.Cols, img.Rows)
        Next


        AssertImgParticle(imgParticles( 0),   16742, 1936, 1464)
        AssertImgParticle(imgParticles( 1),   19480, 1936, 1464)
    ...
    <DataTestMethod()>
    Public Sub TestCroppingWithRect_ZOO80()
        Dim dfw = New DataFileWrapper("DataFiles/ZOO80um_50t60m_diluted5x_8uls_SWST50L60_5min_IIFall_sh high_smrttr SWS SL300 SWS max1000 2025-12-18 13h02.cyz")
        ...
        AssertImgParticleWithRect(imgParticles( 0),  16742, 1936, 1464,  0,   0)
        AssertImgParticleWithRect(imgParticles( 1),  19480, 1936, 1464,  0,   0)
    ...
    ' Add test with ImageMat as well here?
    ' Add a test with live cropped images to make sure that is handled correctly.
